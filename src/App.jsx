...
function LessonMedia({ media }) {
  ...
  return <section className="lesson-media">
    ...
    {parts.length > 0 && <div className="media-parts" aria-label={t('selectedParts')}><span>{t('selectedParts')}</span><div>{parts.map((item, index) => <button key={partKey(item)} className={selectedPartKey === partKey(item) ? 'active' : ''} onClick={() => changePart(partKey(item))}><b>{item.page ? `P${item.page}` : `V${index + 1}`}</b>{item.label}</button>)}</div></div>}
