...
const lessonEn = {
  'p.1':['Python from zero: expressions to testable functions','variables, types, branches, loops, functions, containers, and error messages','Implement and test a pure-Python text statistics tool'],
  'p.2':['Python engineering habits: environments, files, exceptions, and pytest','virtual environments, modules, file I/O, exceptions, assertions, unit tests, and debugging','Turn a one-off script into a reproducible small project'],
  'p.3':['From NumPy arrays to PyTorch tensors','ndarray, shape, axis, broadcasting, views/copies, dtype, and tensors','Vectorize bigram counts and cross-check NumPy against PyTorch'],
  'p.4':['Prerequisite gate: run your first training loop','data, tensors, forward pass, loss, backward pass, updates, tests, and reproducibility','Move from text counts to a tested single gradient update'],
...
const moduleEn = {
  prerequisites:{ title:'Zero-to-Code Prerequisite Sprint', short:'Optional Prerequisites', summary:'Close the Python, testing, NumPy, and PyTorch gap for first-time programmers; experienced learners can test out.', question:'How do you move from running Python to reading, verifying, and changing a tensor training loop?', project:'Python-to-Tensor Starter: a minimal repository with a README, pytest, fixed seeds, and NumPy/PyTorch parity checks.', mastery:['Read common Python syntax and tracebacks','Explain array operations with shape, dtype, and axis','Prove refactors preserve behavior with tests','Run and explain one parameter update'] },
