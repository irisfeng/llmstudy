...
- 双学习路径：LLM 10 个阶段、80 节；World Models 5 个阶段、12 节；进度分别计算
- 中 / EN 整站切换：导航、92 节课程数据、学习页、实验、项目、资料库与账户流程均支持双语
- 67 节课程配有视频研讨：国内模式全部使用 B站；国际模式使用 YouTube 或官方课程、论文、代码仓库，不再回落到 B站
- 长课程精确映射到对应分P；无法合法嵌入的国际材料明确展示官方原始入口
- 92 节课程均有独立双语 URL，并可进入完整学习页：目标、直觉、机制、实践、自测、笔记与掌握门
- 首页与 92 节课程均可生成微信分享卡片，右下角附课程深链二维码，并保留渠道归因参数
...
- 新增可跳过的零基础先修加速带：Python、pytest、NumPy 与 PyTorch 最小训练闭环
- Karpathy《Deep Dive into LLMs》按时间片复用；0.1 与 5.1 只要求相关片段，不再重复整支 3h31m 视频
...
“研究笔记 × 手稿插画”。借鉴 Kaggle / Notion 的涂鸦插画风：浅色主题是纸面手稿、深色主题是黑板粉笔；手绘 SVG 涂鸦（神经网络、世界模型、烧瓶、火箭、靶心）只落在首屏、路线图、便签与掌握门等关键叙事点，入场时一次性“画出”，遵循 `prefers-reduced-motion`。涂鸦组件为原创实现（`src/doodles.jsx`）；保留 anti-slop 纪律：单一强调色、少卡片多轨道、克制动效、避免 AI 紫色渐变与模板式三卡布局。短批注使用手写字体（中文站酷志莽行、英文 Caveat），正文仍用系统字体。
