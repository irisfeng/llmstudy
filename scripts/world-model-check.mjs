...
const genieMechanism = await direct.$eval('.concept-stack', node => node.textContent)
check(genieMechanism.includes('离散的时空 token 网格'), 'Rendered Genie video-tokenizer explanation is not course-specific')
check(genieMechanism.includes('无动作标签视频'), 'Rendered Genie latent-action explanation is not course-specific')
check(!/BPE|执行工具|判断终止/.test(genieMechanism), 'Rendered Genie mechanism still contains an LLM template')
await direct.screenshot({ path:'/tmp/llmstudy-world-lesson-desktop.png', fullPage:false })
await direct.$eval('#study-1', node => node.scrollIntoView({ block:'start', behavior:'instant' }))
await direct.screenshot({ path:'/tmp/llmstudy-genie-mechanism-desktop.png', fullPage:false })
