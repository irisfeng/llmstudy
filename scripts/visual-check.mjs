...
const lessonPathname = await page.evaluate(() => location.pathname)
...
const gradientAfterRun = 'covered by lesson build-and-verify section'
...
await page.click('.study-topbar > button:first-child')
...
mobile.on('console', msg => { if (msg.type() === 'error' && !msg.text().includes('Failed to load resource')) errors.push(msg.text()) })
...
console.log(JSON.stringify({ errors, title: await page.title(), accountFormReady, accountDesktopWidth, accountMobileWidth, accountMobileBottom, mobileAccountPageWidth, searchResults, curriculumVideoBadges, studyTitle, nextTitle, objectiveCount, sectionCount, lessonPathname, workedReveal, quizCorrect, lightTheme, gradientAfterRun, mediaTitle, mediaDeferred, chinaDirect, sourceBadge, mediaParts, selectedPart, partExternal, completedRows, progressLabel, mobileMenuOpen, mobileMediaParts, mobileCurriculumWidth, mobileStudyWidth }, null, 2))
await browser.close()
