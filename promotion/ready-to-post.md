...
标题：做了一个免费、开源的 69 节大模型系统课，想听听大家对课程结构的意见

正文：

我最近把自己学习 LLM 的路线整理成了一个免费、开源的中英双语网站。不是模型 API 合集，而是从反向传播、Token、Attention 和 GPT，一直做到训练预算、DPO、KV Cache、vLLM 与 Agent。

目前有 69 节课，每节尽量包含：要解决的问题、机制拆解、最小实验、自测和掌握门。10 节核心课另外做了可以直接引用的答案、边界条件和一手来源。游客不登录也能学习，进度先保存在本机。
...
把自己学习大模型的路线做成了一个免费网站：69 节中英双语课，从反向传播、Token、Transformer 到训练、DPO、推理部署和 Agent。不是“收藏以后看”，每节都有实践、自测和掌握门。现在最需要的不是点赞，而是认真学习后的纠错和建议。
